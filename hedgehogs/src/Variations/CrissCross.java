 ...
 * @author be8584
 */
public class CrissCross implements GameRules {
	...
	/**
	 * The players play criss-crossing: half of the players run the race from the
	 * Start to the Goal and the other half go from the Goal to the Start. Pieces
	 * which are marked on one side as in variant 33 are not required, but would be
	 * very useful!
	 */
	public CrissCross() {
	...
	public boolean isLegalPlacement ( int row, int col ) {
		if((col == 0 && (row == 0 || row == 2 || row == 4)) || ((col == board_.getWidth()-1) && (row == 1 || row == 3 || row == 5))) {//Make sure it is one of the start columns and rows
